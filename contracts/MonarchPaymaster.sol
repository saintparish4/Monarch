...
contract MonarchPaymaster is IPaymaster, Ownable2Step, ReentrancyGuard {
    ...
    ///
    ///      Do NOT read this off the `postOp` frame in a trace. That frame cost
    ///      11,524 gas on the sponsored path, and setting the constant to 15,000
    ///      on the strength of it broke solvency in every `handleOps` test: the
    ///      EntryPoint finalises `actualGasCost` after `postOp` returns, so the
    ///      unaccounted amount also includes that call's own gas and the
    ///      EntryPoint's refund bookkeeping.
    ///
    ///      Measured the only way that is meaningful — as the deficit a bundle
    ///      leaves behind with no owner buffer. At 15,000 the shortfall was
    ///      4,613 gas on the sponsored path and 4,161 on the deposit path, which
    ///      puts the true overhead near 19,613. Set to 25,000 for headroom.
    ///      Under-set, the paymaster pays part of its own accounting and
    ///      solvency fails; over-set, it overcharges users and apps into the
    ///      owner's excess, which is the safe direction and is what the original
    ///      40,000 guess did.
    uint256 public constant POSTOP_GAS_OVERHEAD = 25_000;
        ...
        returns (bytes memory, uint256)
    {
        ...
        if (rawMode == uint8(Mode.Deposit)) {
            return _validateDeposit(pmData, userOp.sender, maxCost);
        }
        if (rawMode == uint8(Mode.Sponsored)) return _validateSponsored(userOp, maxCost);
    ...
    function _validateDeposit(bytes calldata pmData, address sender, uint256 maxCost)
        internal
        view
        returns (bytes memory, uint256)
    {
        ...
        return (abi.encode(Mode.Deposit, sender, address(0)), SIG_VALIDATION_SUCCESS);
    }
    ...
    ///      staking mandatory.
    function _validateSponsored(PackedUserOperation calldata userOp, uint256 maxCost)
        internal
        view
        returns (bytes memory, uint256)
    {
        ...
        address app = address(
            bytes20(pmData[Constants.APP_OFFSET:Constants.APP_OFFSET + Constants.APP_WIDTH])
        );
        uint48 validUntil = uint48(
            bytes6(
                pmData[Constants.VALID_UNTIL_OFFSET:Constants.VALID_UNTIL_OFFSET
        ...
        uint48 validAfter = uint48(
            bytes6(
                pmData[Constants.VALID_AFTER_OFFSET:Constants.VALID_AFTER_OFFSET
                            + Constants.TIMESTAMP_WIDTH
                ]
            )
        );
        ...
        return (
            abi.encode(Mode.Sponsored, userOp.sender, app),
    ...
    function postOp(
        ...
        (Mode payer, address user, address app) = abi.decode(context, (Mode, address, address));
        uint256 charge = actualGasCost + POSTOP_GAS_OVERHEAD * actualUserOpFeePerGas;
